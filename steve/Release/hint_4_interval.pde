...
void drawArrows(){
  ...
  Rectangle rec4 = getButtonLocation((Integer)trials.get(trialNum+3));
  ...
  arrow(rec3.x+rec3.width/2, rec3.y+rec3.height/2, rec4.x+rec4.width/2, rec4.y+rec4.height/2);
}
...
void drawButton(int i)
{
  ...
  {
    fill(29, 107, 174); // fill next button yellow
    text("next 2", bounds.x, bounds.y+bias);
  }
  else if (trialNum < trials.size()-3 && (Integer)trials.get(trialNum+3) == i && trialNum%hintInterval==0)
  {
    fill(19, 68, 114); // fill next button yellow
    text("next 3", bounds.x, bounds.y+bias);
  }
  else
